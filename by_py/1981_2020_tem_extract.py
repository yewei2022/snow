...
cols=[0,4,5,6,7]
colnames=["station","yy","mm","dd","tmp"]
# filename="SURF_CLI_CHN_MUL_DAY-PRE-13011-200810.TXT"
path_info="F:\\snow_sts_data\\1981-2020-tmp\\data\\"
...
for file in f_list:
    ...
    info_tp['tmp']=info_tp['tmp']*0.1
    # info_tp['dailypre'] = info_tp['dailypre'].apply(lambda x: format(x, '.1f'))
    # 这里保留一位小数的话，na_rep命令无效 
    info_tp.to_csv("F:\\snow_sts_data\\1981-2020-tmp-TP\\"+file[31:37]+".txt",
...
# # #读取tem变成dataframe 
path_pre="F:\\snow_sts_data\\1981-2020-tmp-TP\\"
...
pre.to_csv("F:\\snow_sts_data\\1981-2020\\tmp1981_2020.txt",index = False,
                sep=' ',na_rep=32700)
...
#读取40年80个站点的温度
data0=pd.read_table("F:\\snow_sts_data\\1981-2020\\tmp1981_2020.txt",
...
data3=data2.pivot(index='time', columns='station', values='tmp')
#根据站点顺序重新排序
path_sta='F:\\snow_sts_data\\after_quality_control\\tp_sta_info_by2014.txt'
...
ds = xr.Dataset()
ds['tmp'] = (('time','station'),data5*units('C'))
...
print('保存的数据: \n{}'.format(ds))
ds.to_netcdf('F:\\snow_sts_data\\ERA5\\all\\deal\\tmp_station.nc') 
