...
# df2=pd.read_table("F:\\snow_sts_data\\CTC\\"+pth+"\\u_group.txt",
#                       header=None,sep=',')
# df2.dropna(axis=1, how='any',inplace=True) #删除最后一列空白值
# # u500 = 1.0*(df2 - df2.mean())/df2.std() #标准化，适用于有极端值的情况
# u500=df2

# df3=pd.read_table("F:\\snow_sts_data\\CTC\\"+pth+"\\v_group.txt",
#                       header=None,sep=',')
...
# u500_id.to_csv("F:\\snow_sts_data\\CTC\\"+pth+"\\u500_id_group.txt",
#                     index=False,header=False,sep=" ")
# v500_id= pd.concat([date_id,v500],axis=1)
# v500_id.to_csv("F:\\snow_sts_data\\CTC\\"+pth+"\\v500_id_group.txt",
#                     index=False,header=False,sep=" ")
