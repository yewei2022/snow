...
#%% branch2 开始循环 一小块一小块算 因为数据量太大 内存不够
...
# # branch2 step1 垂直坐标轴插值 我的笔记本不行 内存不够=============================
...
# # #时间维和气压维交换 不用  我需要它后面在最右轴 和高度层匹配
# # var_interp1=np.swapaxes(var_interp, 0, 1)

# # 常数扩展成var_interp 同维数组 为了找每个点和0C最接近的点
...
# # 数组气压层轴，即第2轴滚动到第4轴
# axis=3,代表对第4维进行比较，当气压取不同值时，相同的时间，经纬度点上纬度接近0C
...
#         # print(xi)
#         interp_value=interpolate.interpn((time_index, lat1, lon), arr_hgt11,xi,
#                                          method='linear')
#         arr_hgt2[i,j]=interp_value

# #branch2 step4 增加坐标属性 保存文件=============================================
