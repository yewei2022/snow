...
# #所有循环读入
# varname=['WRPI','IBTI','rh','tcdist','tmp','Wf','Ri']
# for var in varname:
...
#     df2=df1.reset_index()
#     #写入文件
#     df2.to_csv("F:\\snow_sts_data\\regress\\"+folder+"\\"+var+".txt",
...
#     # count = 4
#     a = pd.read_table("F:\\snow_sts_data\\regress\\"+folder+"\\"+
#                       varname[count]+".txt", sep='\s+')
...
# df0=pd.read_table("F:\\snow_sts_data\\regress\\"+folder+"\\R2_pf.txt",sep='\s+')
# def get_sig(x):
#     if x['pf']<0.05: 
#         return 1
#     else:
...
# print('文件夹 "{}" 中线性方程显著的站点数: {}'.format(folder,len(df0[df0['sig']==1])))
# a1 = df0['R2']
# a2 = a1.quantile([0,0.25,0.5,0.75,0.8,0.9,0.95,1]) 
# # print('R2 几分位数: \n{}'.format(a2))
# print('R2 平均值 :{}'.format(a1.mean()))
...
# df1=pd.concat([sta,df0],axis=1,join="inner") #和80个站点信息取交集
# df2=df1.reset_index()
# number_list = [0.3,0.5,0.7]
# for number in number_list:
#     w=number
#     print('R2达到 {} 的站点数量有 {}'.format(w, len(df2[df2['R2']>=w])))

# #写入文件
# df2.to_csv("F:\\snow_sts_data\\regress\\"+folder+"\\R2_sta.txt",
#                     index = False,sep=' ',na_rep=32700, 
