...
# info=pd.read_table("F:\\snow_sts_data\\extrm_events\\extrm_info.txt",sep='\s+',
...
# info['dist']=dist_list
# info.to_csv(path_save+'tcdist_extrm.txt',sep='\t',index=False,na_rep=32700)
...
# # 读数据
# df_tp   = pd.read_table(path_save+'tcdist_extrm.txt',sep = "\t",na_values=32700)
...
# plt.figure(figsize=(4,5))#绘制画布
# sns.distplot(df_tp['dist'],hist = False,kde = True,rug = True,
#               rug_kws = {'color':'g','lw':0.1,'alpha':0.5,'height':0.1} ,
#               # 设置数据频率分布颜色#控制是否显示观测的小细条（边际毛毯）
#               kde_kws={"color": 'g', "lw": 1.5, 'linestyle':'--'},
#               # 设置密度曲线颜色，线宽，标注、线形，#控制是否显示核密度估计图
#               label = 'TP station distance')

# plt.axvline(df_ave,color='black',linestyle=":",alpha=0.8) 
# plt.text(df_ave,0.0003,'%.1fkm' % (df_ave), fontsize=12.5,color = 'black')
# # plt.text(0,0.00082,'(b)', fontsize=13,color = 'black')

# # ylist=np.arange(0,0.0008,0.0001)#.round(1)
# # plt.yticks(ylist)
# plt.ylim([0,0.00085]) #设置坐标上下限
# plt.xlim([0,4000])
# plt.xlabel('Distance',fontsize=12,fontproperties='Helvetica')
...
# plt.savefig(pic_dir+'dist_by_extrm.jpg', dpi=1000, bbox_inches = 'tight')

# # # sns.ecdfplot(df_tp['dist']) #这一步啥意思我忘了
...
# ax.spines['right'].set_color('black')  
# pic_dir="F:\\snow_related\\pic\\tc\\"
# plt.savefig(pic_dir+'dist_box_by_extrm.jpg', dpi=1000, bbox_inches = 'tight')
