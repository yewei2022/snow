...
#%% 
tc_info=pd.read_csv("F:\\snow_sts_data\\TC\\TC_dot_REOF.txt",sep="\s+")
lat=tc_info.groupby(by=['type'])['lat_tc'].mean() #慎用value_counts计数，索引很难搞
lon=tc_info.groupby(by=['type'])['lon_tc'].mean() #慎用value_counts计数，索引很难搞
