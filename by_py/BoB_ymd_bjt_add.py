...
#读取BOB年月日信息，每个个例后面再加一天，转换成BJT，存储
import os
import datetime
import pandas as pd
...
for i in utc:
    ...
    bjt.append(time2.strftime('%Y%m%d%H')[0:8]) #时间变字符串,并提取年月日
    
#%%  step 3   写入文件

tc_ymd_frame=pd.DataFrame(zip(tc_id,bjt),columns=['tc_id','bjt'])
# 去重
tc_ymd_info=tc_ymd_frame.drop_duplicates(["bjt","tc_id"], 
                                          keep='first').reset_index(drop=True)
tc_ymd_info.to_csv("F:\\snow_sts_data\\TC\\BoB_ymd_bjt_add.txt",index = False,
            sep=' ',encoding = "utf-8")
